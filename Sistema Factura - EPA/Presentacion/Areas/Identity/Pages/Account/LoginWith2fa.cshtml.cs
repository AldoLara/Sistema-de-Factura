...
namespace Presentacion.Areas.Identity.Pages.Account
{
    ...
    public class LoginWith2faModel : PageModel
    {
        ...
        public async Task<IActionResult> OnGetAsync(bool rememberMe, string returnUrl = null)
        {
            ...
            if (user == null)
            {
                throw new InvalidOperationException($"No se puede cargar el usuario con autenticación de dos factores.");
            }
        ...
        public async Task<IActionResult> OnPostAsync(bool rememberMe, string returnUrl = null)
        {
            ...
            if (result.Succeeded)
            {
                _logger.LogInformation("Usuario con ID '{UserId}' inició sesión con 2fa.", user.Id);
                return LocalRedirect(returnUrl);
            }
            else if (result.IsLockedOut)
            {
                _logger.LogWarning("Usuario con cuenta ID '{UserId}' bloqueada.", user.Id);
